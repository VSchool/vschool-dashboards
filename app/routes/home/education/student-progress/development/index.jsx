...
export const loader = async () => {
  let localStorage
    ...
    return {
      ...
      progressRecords: JSON.parse(localStorage.getItem('AllStudentProgress'))
    }
  };

export default function DevelopmentPage() {
    const { records, progressRecords } = useLoaderData();

    const devRecords = records.filter(fields => fields["Course Subject"] === "FSJS")
    const devProgress = progressRecords.filter(fields => fields["Course Subject"] && fields["Course Subject"][0] === 'FSJS');

    const [ filteredRecords, setFilteredRecords ] = useState(devRecords);
    const [ filteredProgress, setFilteredProgress ] = useState(devProgress);
    const [ paused, setPaused ] = useState(records.filter(fields => fields["Simple Status"] === "Paused" && fields["Course Subject"] === 'FSJS'))
	...
	function setTimeFrame (timeframe){
        if (timeframe === 'all') {
			setFilteredRecords(devRecords)
			setFilteredProgress(devProgress)
            setPaused(records.filter(fields => fields["Simple Status"] === "Paused" && fields["Course Subject"] === 'FSJS'))
		}else {
			setFilteredRecords(devRecords.filter(fields => {
			...
			setFilteredProgress(devProgress.filter(fields => {
            ...
            setPaused(records.filter(fields => fields["Simple Status"] === "Paused" && (timeframe === 'week' ? fields['Time to Start'] < 7 : fields['Time to Start'] < (timeframe * 30)) && fields["Course Subject"] === "FSJS"))
		}
    }

    return (
        <main>
        <h1>Development Student Progress Records</h1>
		<div className="flex">
