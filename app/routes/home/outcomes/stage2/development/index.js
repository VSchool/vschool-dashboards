...
export default function AllStage2Page() {
    ...
    return (
        <main>
            <h1>Stage 2 Development Records</h1>
            <div className="flex">
            ...
            <br />
            <div>
                <p>Active: {filteredRecords.filter(fields => fields['Course Subject'][0] === 'FSJS').length}</p>
                <p>Has Project: {filteredRecords.filter(fields => fields["Status"] === "In progress" && fields['Course Subject'][0] === 'FSJS').length}</p>
                <p>Needs Project: {filteredRecords.filter(fields => fields["Status"] !== "In progress" && fields['Course Subject'][0] === 'FSJS').length}</p>
                <p>Paid Projects: {filteredRecords.filter(fields => fields["Compensation Rate"] && fields['Course Subject'][0] === 'FSJS').length}</p>
                <p>Volunteer Project: {filteredRecords.filter(fields => !fields["Compensation Rate"] && fields['Course Subject'][0] === 'FSJS').length}</p>
                <p>Internships: {filteredRecords.filter(fields => fields["Type"] === "Internship" && fields['Course Subject'][0] === 'FSJS').length}</p>
                <p>PT/Contract: {filteredRecords.filter(fields => fields["Type"] === "Contractor" && fields['Course Subject'][0] === 'FSJS').length}</p>
            </div>
        </main>
    );
}